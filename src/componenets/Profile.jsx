...
import { updateUserWords, getUserProfile } from '../common/api';
...
function Profile() {
    const { token, user, logout, updateUser, updateWords } = useAuth();
    const navigate = useNavigate();

    async function loadUserProfile() {
        if (token) {
            let userData = await getUserProfile(token, "me");
            if (userData?.error) {
                alert("Session expired , login again !")
                logout();
            }
            else updateUser(userData.user);
        }
    }
    useEffect(() => {
        loadUserProfile();
    ...
    return (
        <m.div>
            {!user ? <div>loading ... </div> :
                <div className='p-4 flex flex-col md:flex-row-reverse gap-4  h-screen m-2'>
